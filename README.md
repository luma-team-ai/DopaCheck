...
**기술 스택**: Python 3.10+ · Flask · Jinja2 · MariaDB · OAuth(Google·Kakao) · Claude API · Cloudtype
...
>
> 📋 **[진행 현황 & 내 할 일 보드 → docs/STATUS.md](docs/STATUS.md)** · 🎯 **[발표 D-2.5 역할 분배 →](docs/STATUS.md#sprint)** · 📑 상세 스펙 [PRD](docs/PRD.md)
...
> **Python 3.10+ 필수** (배포 타깃 `3.10.2` — `runtime.txt`·`.python-version`에 고정). 소셜로그인 의존성 `authlib>=1.7`이 Python 3.10+를 요구하므로 3.9에서는 설치·실행 불가.

```bash
git clone https://github.com/luma-team-ai/dopacheck.git
cd dopacheck

python3.10 -m venv .venv && source .venv/bin/activate   # 3.10+ 인터프리터 사용
...
> 🎯 **지금 당장 뭘 해야 하나? (발표 D-2.5)** → **[Ver1.2 마무리 스프린트 역할 분배](docs/STATUS.md#sprint)** — 각자 할 일·우선순위 보드. 아래 표는 **영구 도메인 담당**(변하지 않는 책임 영역).
