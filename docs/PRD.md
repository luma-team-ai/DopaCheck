...
> ⚠️ **인프라 변경 (2026-07-02)**: 본문의 Cloudtype 배포·DB 언급은 개발 당시 기록이다. 현재 운영은 **OCI 자체 서버**(https://dopacheck.luma200ok.com, Docker+nginx)로 이전됐다 — 최신 배포 절차는 `docs/guide/DEV_GUIDE.md` §6 참조.
...
4. 챌린지 달성 시 도파민 점수에 감점(-5점)이 반영된다. (위험도 감소)
...
- FR-27: 점수 산출 공식: `배달 기여(40%) + 시간 기여(40%) + 챌린지 감점(최대 -20%)`. 점수가 높을수록 도파민 위험도↑. 배달·SNS 소비 많을수록 점수↑, 챌린지 달성 시 감점으로 위험도↓.
...
- FR-38: 챌린지 달성 시 도파민 점수에 -5점 감점이 반영되어야 한다. (위험도를 낮추는 방향, `challenge_bonus`에 음수로 저장)
...
- [x] Given 챌린지를 달성하면, Then 도파민 점수에 -5점 감점이 반영된다. (위험도 감소)
...
| challenge_bonus | INT (0~-20) | 챌린지 감점 (음수 저장, 달성할수록 위험도↓) |
...
// 높을수록 도파민 위험도↑. challenge_bonus는 음수(감점) 저장.
```
