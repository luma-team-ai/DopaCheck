...
> **상태: 완료.** 발표(2026-06-17경) 종료, **열린 GitHub 이슈 0건**. 아래 마무리 스프린트 잔여 항목(#131·#129·#130·#128)은 **전부 CLOSED**(2026-06-16).
> 이 보드는 개발 기간의 역할 분배 기록이며, 최신 코드 단위 이력은 아래 [마지막 머지 PR](#마지막-머지-pr) 섹션을 본다.

### ✅ 마무리 스프린트 최종 결과
- **김승현**: #131 랭킹/percentile 시연 더미 데이터 ✅ (재현용 `db/seed_demo.sql`) · `/admin` 통계 대시보드 ✅ (#106)
- **김관영**: #129 전체 흐름 통합 테스트 + OCR 정확도 QA ✅ (CLOSED)
- **이은석**: #130 모바일(Chrome/Safari) QA + 배포 안정화 ✅ (CLOSED)
- **오영석**: #73 챌린지 완료 처리 · #74 join TOCTOU · #24 calorie 방어 ✅
- **허남**: history·challenge Stitch 마이그레이션 ✅ · 히스토리 상세 환산(#127)
- **정재봉**: PRD Ver1.3 정합(#109) · #128 time CSRF 회귀 테스트(#168) ✅ · 검수·머지 오케스트레이션(상시)

> ⚠️ **테스트 참고**: `pytest` 203 PASS / 1 skip / **1 fail**. 실패 1건(`test_challenge.py::test_이번주_가입_judge지난주_완료_안됨`)은 `get_week_ranges()`(실제 `date.today()`)를 mock하지 않아 하드코딩 날짜(6/15 주)와 실제 달력이 어긋날 때만 실패하는 **테스트의 시간 커플링 결함**(프로덕션 로직 무관). 머지 당시엔 우연히 통과. → 후속 테스트 보강 권장.
...
| 시드 더미 데이터 20건 (랭킹/percentile 시연) | ✅ 완료 (**#131**) — 재현용 `db/seed_demo.sql`(시연 전용, 재실행 안전·주차 자동계산). `db/seed.sql`은 기본 챌린지 7종(운영 실행용) | 김승현 |
| 운영 마이그레이션 001~004 (role·challenges dedup·uc index·challenge_bonus check) | ✅ 파일 머지 (운영 ALTER 적용 완료 전제) | 김승현/정재봉 |
...
### P2 (통합 — Day 5~6) — ✅ 완료
- [x] 전체 흐름 통합 테스트 (#129) · [x] 데모 시나리오 완주 · [x] 모바일 QA (#130, Chrome/Safari)
...
| ~~#131 랭킹 시연 더미 20건~~ | ✅ **CLOSED** — 재현용 `db/seed_demo.sql` 커밋(시연 전용, 운영 실행 금지) |
| ~~#128 time CSRF 403 테스트~~ | ✅ **#168로 해소** — CSRF 미전송/오토큰 403 회귀 테스트 추가 |
| ~~#129 통합테스트·OCR QA~~ | ✅ **CLOSED** — 전체 흐름 통합 + 영수증 OCR 정확도 점검 완료 |
| ~~#130 모바일 QA·배포 안정화~~ | ✅ **CLOSED** — Chrome/Safari 모바일 QA + 배포 점검 완료 |
| pytest 시간 커플링 1건 | `test_challenge.py::test_이번주_가입_judge지난주_완료_안됨`이 `get_week_ranges()` 미mock으로 특정 주(6/15)에만 통과 — 프로덕션 무관, 테스트 보강 후속 권장 |
| 운영 마이그레이션 적용 | 001~004 파일 머지 완료. **운영 Cloudtype DB ALTER 적용 전제**(003 인덱스 → #113 TOCTOU 발효 조건, 004 challenge_bonus CHECK) |
