...
데이터는 안전하다(두 번째 실행은 업데이트 대상이 없음). 다만 워커마다 같은 cron이
기동돼 집계 쿼리·완료 로그가 워커 수만큼 중복되므로, advisory lock(GET_LOCK)으로
한 워커만 실제 판정을 수행하도록 한다(나머지는 즉시 스킵).
"""
import logging
...
# 멀티워커 중복 실행 차단용 advisory lock 이름. GET_LOCK은 커넥션 스코프이므로
# 반드시 같은 커넥션에서 잡고 RELEASE_LOCK으로 해제해야 한다(풀 반납 시 커넥션이
# close되지 않아 미해제 시 락이 영구 점유됨).
_LOCK_NAME = "dopacheck_settle_last_week"


def settle_last_week_challenges() -> int:
    """지난 주 종료 후 미판정 상태로 남은 챌린지를 일괄 달성 판정한다.

    gunicorn 멀티워커가 동일 cron을 동시에 띄우므로, advisory lock을 획득한
    워커만 판정을 수행한다(timeout=0 → 미획득 시 즉시 스킵, 대기 없음).

    Returns:
        완료 처리된 user_challenges 행 수. 락 미획득 시 0.
    """
    ...
    completed_count = 0
    with db() as cursor:
        # timeout=0: 다른 워커가 이미 보유 중이면 즉시 0 반환(falsy) → 스킵.
        # NULL(에러) 도 falsy로 처리해 안전하게 스킵한다.
        cursor.execute("SELECT GET_LOCK(%s, 0) AS got", (_LOCK_NAME,))
        lock_row = cursor.fetchone()
        if not lock_row or not lock_row.get("got"):
            logger.info("배치 락 미획득 — 다른 워커가 실행 중이므로 스킵")
            return 0

        try:
            completed_count = _settle(cursor, gte_at, lt_at, week_end)
        finally:
            # 같은 커넥션에서 명시적 해제 — 풀 반납만으로는 락이 풀리지 않는다.
...
def _settle(cursor, gte_at: str, lt_at: str, week_end: str) -> int:
    """락을 획득한 워커가 수행하는 실제 판정 로직. 완료 처리 행 수를 반환한다."""
    completed_count = 0
    cursor.execute(
