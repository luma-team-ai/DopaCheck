...
def generate_tip(score: int, avg_min: int, delivery_total: int) -> dict:
    ...
    print(f"\n{CLR_HEADER}============================================================{CLR_END}")
    print(f"{CLR_BLUE}[AI 맞춤 팁 생성 시작]{CLR_END}")
    print(f" - 입력 데이터: 도파민 점수={score}/100, SNS·게임={avg_min}분, 배달={delivery_total}원")
    print(f" - 사용 모델: {MODEL_COMMENT}")
    ...
    prompt = (
        ...
        f"점수가 낮을수록 적극적인 행동 교정을, 점수가 높을수록 현재 상태 유지를 독려하는 팁을 작성해주세요. "
    ...
    try:
        print(f"[*] Anthropic API 호출을 전송합니다...")
        response = get_client().messages.create(
            model=MODEL_COMMENT,
            max_tokens=512,
            system=_TIP_SYSTEM,
        ...
        print(f"{CLR_GREEN}[+] API 호출에 성공하였습니다.{CLR_END}")
        ...
        if "title" in data and "content" in data:
            print(f"{CLR_GREEN}[✔] JSON 파싱 및 유효성 검사 성공!{CLR_END}")
            print(f" - 생성된 팁 제목: {data['title']}")
            print(f"{CLR_HEADER}============================================================{CLR_END}\n")
            return data
            
        print(f"{CLR_WARNING}[!] 응답에 필수 필드('title', 'content')가 누락되었습니다. 기본 팁을 반환합니다.{CLR_END}")
        print(f"{CLR_HEADER}============================================================{CLR_END}\n")
        return default_tip
    except Exception as e:
        print(f"{CLR_FAIL}[-] AI 팁 생성 중 에러가 발생했습니다.{CLR_END}")
        print(f"{CLR_FAIL}[에러 원인] {e}{CLR_END}")
        print(f"[*] 에러 상세 정보:")
        traceback.print_exc(file=sys.stdout)
        print(f"{CLR_WARNING}[!] 기본(Fallback) 팁 데이터를 전송합니다.{CLR_END}")
        print(f"{CLR_HEADER}============================================================{CLR_END}\n")
        return default_tip
