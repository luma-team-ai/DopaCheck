...
def parse_receipt(image_bytes: bytes) -> dict:
    ...
    prompt = (
        ...
        "【추출 규칙】\n"
        "1. '상품명'은 컬럼 헤더입니다. items에 넣지 마세요.\n"
        "2. '상품명' 헤더 아래 실제 음식명(예: 카레덮밥, 돈까스 등)만 items에 포함하세요.\n"
        "3. '- 세트:', '- 덮밥:', '- 추가:' 등 하이픈(-)으로 시작하는 옵션 줄은 "
        "바로 위 음식명에 괄호로 합쳐주세요. 예) 카레덮밥 미니면세트(미니우동)\n"
        "4. 공급가액·부가세·합계·할인·적립금·신용카드·승인번호·가맹점 등 "
        "결제/세금 관련 항목은 items에 포함하지 마세요.\n"
        "5. total_price는 '합계' 또는 '제금액' 란의 숫자를 사용하세요.\n"
        "6. 배달비(배달료) 항목이 있으면 delivery_fee에 넣고, 없으면 0으로 설정하세요.\n"
        "7. 가격이 불명확하면 0으로 처리하세요.\n\n"
